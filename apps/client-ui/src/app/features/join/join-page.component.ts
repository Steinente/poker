...
import type { LobbySummary } from '@poker/shared'
      ...
      <div class="panel" style="max-width: 620px; margin: 0 auto; width: 100%;">
        ...
        @if (!deviceSwitchToken) {
          ...
          <div class="panel" style="margin-top: 12px;">
            <span class="muted">{{ 'deviceSwitch.joinHint' | t }}</span>
          </div>
        }
...
export class JoinPageComponent implements OnInit, OnDestroy {
  ...
  deviceSwitchToken =
    this.route.snapshot.queryParamMap.get('deviceSwitch')?.trim() ?? ''
  private triedDeviceSwitch = false
  ...
  private tryCompleteDeviceSwitch() {
    if (!this.deviceSwitchToken || this.triedDeviceSwitch) {
      return
    }

    this.triedDeviceSwitch = true
    this.facade.completeDeviceSwitch(this.deviceSwitchToken)
  }
}
