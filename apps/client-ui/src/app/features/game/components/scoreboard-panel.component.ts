...
@Component({
  selector: 'poker-scoreboard-panel',
  standalone: true,
...
export class ScoreboardPanelComponent implements OnChanges {
  ...
  displayedA11yRoundNumbers() {
    return this.playedRoundNumbers()
  }

  totalScore(playerId: string) {
    const rounds = this.playedRoundNumbers()
    for (let i = rounds.length - 1; i >= 0; i--) {
      const entry = this.scoreEntry(playerId, rounds[i])
  ...
  pointsValue(playerId: string, round: number) {
    return this.scoreEntry(playerId, round)?.total ?? null
  }

  scoreDisplayValue(value: number | null) {
