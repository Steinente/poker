...
@Component({
  selector: 'poker-game-settings-panel',
  standalone: true,
...
export class GameSettingsPanelComponent {
  ...
  @Input({ required: true }) raiseInputMode: RaiseInputMode = 'to'
  @Input({ required: true }) onRaiseInputModeChange!: (
    mode: RaiseInputMode,
  ) => void
  ...
  changeRaiseInputMode(mode: RaiseInputMode | string) {
    this.onRaiseInputModeChange(mode === 'by' ? 'by' : 'to')
  }
