import type { SpecialCardKey, PokerGameState } from '@poker/shared'
import {
...
export function applyRoundStartState(state: PokerGameState) {
  ...
  // Special rule: if Dark Eye is included and we're in the final round with exactly 1 card
  // remaining after dealing, no trump can be selected to allow Dark Eye to draw cards
  const hasDarkEye = state.config.includedSpecialCards.includes(
    SPECIAL_CARD_KEY.darkEye,
  )

  const maxPossibleRounds = calculateMaxPossibleRounds(
    round.players.length,
    state.config.includedSpecialCards,
  )

  let isDarkEyeFinalRound = false
  if (hasDarkEye && round.roundNumber === maxPossibleRounds) {
    // Calculate remaining cards after dealing in the final round
    const deckSize = 60 + state.config.includedSpecialCards.length

    const cardsUsedInFinalRound = round.roundNumber * round.players.length
    const remainingCards = deckSize - cardsUsedInFinalRound

    // If exactly 1 card remains, no trump card should be set
    isDarkEyeFinalRound = remainingCards === 1
  }

  const pushSystemLog = (
    messageKey: PokerGameState['logs'][number]['messageKey'],
  ...
  const beginSelectTrumpSuitDecision = (
    special: string,
    pendingLogKey: PokerGameState['logs'][number]['messageKey'],
  ...
  if (round.trumpCard?.type === 'wild') {
    beginSelectTrumpSuitDecision('wild', 'game.trump.selection.pending')
    return
  }
...
export function buildInitialState(
  ...
  options?: { randomizerPoolSpecialCards?: SpecialCardKey[] },
): PokerGameState {
  ...
  const state = createInitialGameState({
    ...
    randomizerPoolSpecialCards:
      options?.randomizerPoolSpecialCards ??
      (config.specialCardsRandomizerEnabled
        ? [...config.includedSpecialCards]
        : []),
    players,
  })
