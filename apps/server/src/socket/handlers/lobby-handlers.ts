...
export const registerLobbyHandlers = ({
  ...
  socket.on('lobby:reopen', async (payload) => {
    try {
      const input = reopenLobbySchema.parse(payload)
      const lobby = await lobbyService.reopenFinishedLobby(input)
      ...
      emitError(
        socket,
        error instanceof Error ? error.message : 'error.gameStartFailed',
      )
    }
  })

  socket.on('lobby:joinReplay', async (payload) => {
    try {
      const input = joinReplayLobbySchema.parse(payload)
      const lobby = await lobbyService.joinReplayLobby(input)
