...
export interface RuntimeCardEffectLookup {
  ...
  shapeShifterMode?: 'wild' | 'jester' | null
...
const getShapeShifterMode = (
  runtimeEffect?: RuntimeCardEffectLookup | null,
): 'wild' | 'jester' => runtimeEffect?.shapeShifterMode ?? 'jester'
...
export const classifyCard = (
  ...
  if (isWildCard(card)) return classified(card, CARD_CLASS.wild, 1000, null)
  ...
  if (isSpecialCard(card)) {
    ...
    if (card.special === SPECIAL_CARD_KEY.shapeShifter) {
      ...
      return mode === 'wild'
        ? classified(card, CARD_CLASS.wild, 1000, null)
